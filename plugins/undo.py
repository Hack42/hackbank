...
class undo:
    ...
    def hook_undo(self,(transID,totals,receipt,beni)):
        ...
        for usr in totals:
            if totals[usr]>0:
                self.master.receipt.add(True,totals[usr],'Undo '+str(transID),1,usr,'undo')
            else:
                self.master.receipt.add(False,0-totals[usr],'Undo '+str(transID),1,usr,'undo')
    ...
    def doundo(self,text):
        ...
        try:
            transID=int(text)
            if transID in self.undo:
                self.master.callhook('undo',(transID,self.undo[transID]['totals'],self.undo[transID]['receipt'],self.undo[transID]['beni']))
                return True
            else:
                self.listundo()
                return True
        ...
        try:
            ...
            if transID in self.undo:
                ...
                for rr in receipt:
                    ...
                    if rr['beni'] != beni:
                        nowbeni=rr['beni']
                    self.master.receipt.add(rr['Lose'],rr['value'],rr['description'],rr['count'],nowbeni,rr['product'])
                return True
            else:
                self.listundo()
                return True
    ...
    def listundo(self,restore):
        ...
        for transID in self.undo.keys():
            ...
            txt+=time.strftime("%Y-%m-%d %H:%M:%S",time.localtime(transID+1300000000))
