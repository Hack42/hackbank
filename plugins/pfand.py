...
class pfand:
    ...
    def listpfand(self):
        ...
        for prod in self.products:
             ...
             custom.append({'text':prod,'display':'Pfand '+desc, 'right':self.products[prod], 'rightclass': 'green' })
    ...
    def pfand(self,text):
        if text in self.products:
          self.master.receipt.add(False,self.products[text],'Pfand return '+text,1,None,'return_'+text)
          return True
        elif text=="abort":
          ...
          self.listpfand()
          self.master.send_message(True,'message',"Unknown product, what product are you returning?")
          return True
