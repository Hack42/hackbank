...
class give:
    ...
    def who(self,text):
        if text in self.master.accounts.accounts:
            ...
            self.master.send_message(True,'message','How much do you want to give to '+self.userto+'?')
    ...
    def amount(self,text):
        try:
            ...
            if value>0 and value<1000:
                ...
                self.master.send_message(True,'message','Why are you giving '+str(self.value)+' to '+self.userto+'?')
            ...
            if text=="abort":
                ...
                return True
            self.master.donext(self,'amount')
            self.master.send_message(True,'message','Not a valid number! How much do you want to give to '+self.userto+'?')
            self.master.send_message(True,'buttons',json.dumps({'special':'numbers'}))
            return True
    ...
    def reason(self,text):
        if text=="abort":
            ...
            return True
        self.myreason=text
        self.master.receipt.add(True,self.value,'Give to '+self.userto+' ('+self.myreason+')',1,None,'give')
        self.master.receipt.add(False,self.value,'Given from -you- ('+self.myreason+')',1,self.userto,'give')
        return True
