...
class declaratie:
    ...
    def who(self, text):
        if text in self.master.accounts.accounts:
            ...
            if self.soort == "verkoop":
                ...
                self.master.send_message(
                    True, "message", "How much money do you want back?"
                )
            self.master.send_message(
        ...
        try:
            ...
            if self.soort == "verkoop":
                ...
                self.master.send_message(
                    True,
                    "message",
                    "Not a valid number! How much money do you want back?",
                )
            self.master.send_message(
                True, "buttons", json.dumps({"special": "numbers"})
            )
    ...
    def askbar(self, error):
        ...
        if self.soort == "declaratie":
            self.master.send_message(
                True, "message", error + "How much do you want on your bar account?"
            )
        else:
            self.master.send_message(
                True, "message", error + "How much do you want from your bar account?"
            )
        custom = []
        custom.append({"text": "0", "display": "None"})
        custom.append(
            {"text": str(self.value), "display": "Everything (E %.2f)" % self.value}
        )
        self.master.donext(self, "runasbar")
        self.master.send_message(
    ...
    def askcash(self, error):
        ...
        if self.soort == "declaratie":
            self.master.send_message(
                True, "message", error + "How much do you want in cash?"
            )
        else:
            self.master.send_message(
                True, "message", error + "How much do you give in cash?"
            )
        custom = []
        custom.append({"text": "0", "display": "None"})
        custom.append(
            {
                "text": str(self.value - self.asbar),
                "display": "Everything (E %.2f)" % (self.value - self.asbar),
            }
        )
        self.master.donext(self, "runascash")
        self.master.send_message(
    ...
    def askbank(self, error):
        ...
        if self.soort == "declaratie":
            self.master.send_message(
                True, "message", error + "How much do you want in your bankaccount?"
            )
        else:
            self.master.send_message(
                True, "message", error + "How much do you send from your bankaccount?"
            )
        custom = []
        custom.append({"text": "0", "display": "None"})
        custom.append(
            {
                "text": str(self.value - self.asbar - self.ascash),
                "display": "Everything (E %.2f)"
                % (self.value - self.asbar - self.ascash),
            }
        )
        self.master.donext(self, "runasbank")
        self.master.send_message(
    ...
    def bon(self):
        self.master.POS.printdeclaratie(
            (self.wie, self.soort, self.reden, self.asbar, self.ascash, self.asbank)
        )
        ...
        if text == "yes":
            if self.asbar > 0:
                self.master.receipt.add(
                    False, self.asbar, "Declaratie", 1, self.wie, "declaratie"
                )
                self.master.callhook("checkout", self.wie)
                self.master.callhook("endsession", self.wie)
            if self.asbar < 0:
                self.master.receipt.add(
                    True, 0 - self.asbar, "Declaratie", 1, self.wie, "declaratie"
                )
                self.master.callhook("checkout", self.wie)
                self.master.callhook("endsession", self.wie)
            if self.ascash != 0:
