...
class market:
    ...
    def help(self):
        return {"addmarket": "Market: Add alias", "delmarket": "Market: Remove a product", "changemarket": "Market: Change Price", "market": "Market: Products"}
    ...
    def readproducts(self):
        ...
        for line in lines:
            ...
            if len(parts) == 5 and not line.startswith('#'):
                ...
                name = aliases.pop(0)
                self.products[name] = {'price': float(parts[2]), 'space': float(
                    parts[3]), 'description': parts[4], 'aliases': aliases, 'user': parts[0]}
                for alias in aliases:
                    self.aliases[alias] = name
        for prod in self.products:
            self.master.send_message(True, 'market/' + prod, json.dumps(self.products[prod]))
    ...
    def input(self, text):
        prod = self.lookupprod(text)
        if prod:
            self.master.receipt.add(True, self.products[prod]['price'], 'Give to ' + self.products[prod]['user'] + ' (Market:' + self.products[prod]['description'] +')', self.master.products.times, None, prod)
            self.master.receipt.add(
                False, self.products[prod]['price'], 'Given from -you- (Market:' + self.products[prod]['description']+')', self.master.products.times, self.products[prod]['user'], prod)
            self.master.receipt.add(True, self.products[prod]['space'], self.products[prod]['description'], self.master.products.times, None, prod)
            self.master.products.times = 1
            return True
        elif text=="market":
            custom=[]
            for prod in self.products:
                custom.append({'text': prod,'display': self.products[prod]['description'], 'right': "%0.2f (%0.2f)" % (self.products[prod]['price']+self.products[prod]['space'], self.products[prod]['space'])})
