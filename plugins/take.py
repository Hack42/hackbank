...
class take:
    ...
    def who(self,text):
        if text in self.master.accounts.accounts:
            ...
            self.master.send_message(True,'message','User to take from or amount to end? ('+', '.join(self.totakefrom)+')')
            self.master.send_message(True,'buttons',json.dumps({'special':'accountsamount'}))
            return True
        elif text=="abort":
            ...
            try:
                ...
                traceback.print_exc()
        self.master.donext(self,'who')
        self.master.send_message(True,'message','Unknown User; User to take from? ('+', '.join(self.totakefrom)+')')
        self.master.send_message(True,'buttons',json.dumps({'special':'accountsamount'}))
        return True
    ...
    def reason(self,text):
        ...
        for van in self.totakefrom:
            self.master.receipt.add(True,self.peruser,'Taken by -you- ('+self.myreason+')',1,van,'take')
            if vanline!="": vanline+=", "
            vanline+=van
        self.master.receipt.add(False,self.peruser*len(self.totakefrom),'Taken from '+vanline+' ('+self.myreason+')',1,None,'take')
        return True
