...
def test_eigendom(_cups):
    ...
    expected_calls = [
        ...
        call(True, 'buttons', '{"special": "accounts"}')  # Make sure the JSON string matches exactly
    ]
    ...
    expected_calls = [call(True, 'message', 'How many do you want?'), call(True, 'buttons', '{"special": "numbers"}')]
    assert sticky.master.send_message.call_args_list == expected_calls
...
def test_barcode(_cups):
    ...
    product_data = {'aliases': ['1234'], 'price': 10.0, 'description': 'Test Description'}
    master.products.products = Mock()
    master.products.products.get.side_effect = lambda k: product_data if k == product_alias else None
